class Code128 {
    constructor(id, codeA, codeB, codeC, pattern) {
        this.id = id;
        this.codeA = codeA;
        this.codeB = codeB;
        this.codeC = codeC;
...
export class Code128Reader {
    constructor(width, height) {
        ...
        this.binarize = this.gpu.createKernel(function (image, maxmin) {
            let min2 = 1000;
            let max2 = -1000;
            for (let y = -1; y < 2; y++) {
                for (let x = -1; x < 2; x++) {
        ...
        this.binarize2 = this.gpu.createKernel(function (image, maxmin) {
            let min2 = 1000;
            let max2 = -1000;
            for (let y = -1; y < 2; y++) {
                for (let x = -1; x < 2; x++) {
    ...
    analyze(image) {
        ...
        const binarized = this.binarize(grayImage, maxmin);
        ...
        for (let i = 16; i < this.height - 16; i++) {
            ...
            if (parser.isOK) {
                ...
                result4.push(readResult);
            }
        }
        if (result4.length > 0) {
            console.log(result4.map(item => item.y + " " + item.result));
            const res = mode(result4.map((item) => item.result));
            const result5 = result4.filter((item) => item.result === res);
            if (result5.length > 5) {
                return [res, result5];
            }
        }
